...
fn ensure_worker_registered(
    ...
    THIS_WORKER.with(|cell| {
        ...
        GLOBAL_EMIT_REGISTRY
            .lock()
            .expect("WorkerEmitSlots: global registry lock poisoned")
            .entry(runtime_id)
            .or_default()
...
pub(crate) fn emit<M: Message>(
    ...
    let worker = ensure_worker_registered(runtime_id, num_message_types);
...
pub(crate) fn drain_into(
    ...
    let registry = GLOBAL_EMIT_REGISTRY
        .lock()
        .map_err(|_| ECSError::Internal(crate::engine::error::InternalViolation::PoisonedLock))?;
