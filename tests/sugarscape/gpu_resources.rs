...
use abm_framework::gpu::{GPUBindingDesc, GPUContext, GPUResource};
use abm_framework::{ECSError, ECSResult, ExecutionError};
...
impl GPUResource for SugarGrid {
    ...
    fn upload(&mut self, _: &GPUContext) -> ECSResult<()> {
        Ok(())
    }
    fn download(&mut self, _: &GPUContext) -> ECSResult<()> {
        Ok(())
    }
...
impl GPUResource for AgentIntentBuffers {
    ...
    fn create_gpu(&mut self, ctx: &GPUContext) -> ECSResult<()> {
        ...
        self.agent_target = Some(ctx.device.create_buffer_init(
    ...
    fn download(&mut self, ctx: &GPUContext) -> ECSResult<()> {
        read_buffer_u32(
            ctx,
            self.agent_target.as_ref().unwrap(),
            &mut self.target_cpu,
        )?;
